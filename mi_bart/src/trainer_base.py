...
class TrainerBase(object):
    ...
    def create_config(self):
        ...
        config.num_beams = 1 # self.args.num_beams
        config.task_specific_params['summarization_xsum']['num_beams'] = 1
        # edit end.
        ...
        config.n_images = 4 # it was 2 earlier, modifying to 4 for K related query ijcai rebuttal
    ...
    def load_checkpoint(self, ckpt_path):
        ...
        # modified to load state dict except image order embeddings
        model_dict = self.model.state_dict()

        # filter out unnecessary keys
        pretrained_dict = {k: v for k, v in state_dict.items() if 'img_order_embedding' not in k}
        ...
        results = self.model.load_state_dict(model_dict, strict=False)

        # results = self.model.load_state_dict(state_dict, strict=False)
        # till here modified, if want to revert to original version, uncomment the above line and comment 193-201
