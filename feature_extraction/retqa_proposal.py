...
vg_classes = []
with open('/data1/abhiram/webqa/VL-T5/feature_extraction/vg_data/objects_vocab.txt') as f:
    for object in f.readlines():
        vg_classes.append(object.split(',')[0].lower().strip())

vg_attrs = []
with open('/data1/abhiram/webqa/VL-T5/feature_extraction/vg_data/attributes_vocab.txt') as f:
    for object in f.readlines():
...
def build_model():
    ...
    from pathlib import Path
    cfg.MODEL.WEIGHTS = str(Path.home().joinpath('frcnn_weights/faster_rcnn_from_caffe_attr.pkl'))
...
class WebQAImageDataset(Dataset):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--retvqa_dir', type=str,
                        default='/data1/abhiram/webqa/VL-T5/datasets/webqa/webqa_stitched_imgs/')
    ...
    retvqa_dir = Path('/data1/abhiram/webqa/VL-T5/datasets/webqa_stitched').resolve()
    # flickr_img_dir = flickr_dir.joinpath('flickr30k_images/').joinpath(SPLIT2DIR[args.split])
    retvqa_img_dir = Path('/data1/abhiram/webqa/VL-T5/datasets/webqa/webqa_stitched_imgs').resolve()
    dataset_name = 'retVQAImages'

    out_dir = retvqa_dir.joinpath('features')
    if not out_dir.exists():
        out_dir.mkdir()

    print('Load images from', retvqa_img_dir)
    print('# Images:', len(list(retvqa_img_dir.iterdir())))

    dataset = WebQAImageDataset(retvqa_img_dir)
